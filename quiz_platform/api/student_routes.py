...
async def get_quiz_for_attempt(
    ...
    # 1️⃣ Check quiz exists and is published
    ...
    # 3️⃣ Get active questions
    questions = db.query(Question).filter(
        Question.quiz_id == quiz_id,
        Question.is_active == True
    ).order_by(Question.question_order).all()

    # 4️⃣ Format questions safely
    formatted_questions = []
    for q in questions:
        try:
    ...
    return {
        ...
        "time_limit_minutes": quiz.total_questions * 2  # ✅ REQUIRED (or real DB field if you have one)
    }
...
@router.get("/attempt/{attempt_id}/result", response_model=QuizResult)
async def get_attempt_result(
    ...
    service = StudentService(db)
    return service.get_attempt_result(attempt_id, current_user.id)
